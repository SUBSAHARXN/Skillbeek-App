...
interface ReceiveSkillsAddViewProps {
  onBack: () => void;
  onNext: () => void;
}

export function ReceiveSkillsAddView({ onBack, onNext }: ReceiveSkillsAddViewProps) {
  return (
    <div className="w-full max-w-[384px] h-[812px] bg-[var(--Surface-Primary-Background)] rounded-[32px] overflow-hidden relative flex flex-col mx-auto shadow-2xl">
      ...
      <div className="flex-1 overflow-y-auto flex flex-col items-center pb-[34px]">
        <div className="px-[16px] w-full flex flex-col items-center">
          {/* Back Button - Positioned exactly like in AddSkillView */}
          ...
          <div className="w-full flex flex-col gap-[12px] mt-[16px] mb-[44px]">
            <h1 className="font-['Nunito'] font-bold text-[var(--Text-Primary-heading-1)] text-[28px] leading-[36px] tracking-[-1.2px]">
              Select up to three skills you'd like to receive in return.
            </h1>
            <p className="font-['Nunito'] font-medium text-[var(--Text-Primary-Body)] text-[16px] leading-[24px] tracking-[0.1px]">
              Let us know what you're looking for, and we'll connect you with the right people
            </p>
          </div>
        ...
        <div className="w-full flex justify-center px-[16px] mb-[64px]">
          <button
            onClick={onNext}
            className="w-full h-[48px] flex items-center justify-center gap-[6px] bg-[var(--Surface-UI-surface-Surface-Universal-alternate-lighter)] rounded-[16px] text-[var(--Text-Primary-Body-alt)] font-['Nunito'] font-bold text-[16px] leading-[24px] tracking-[0.16px] shadow-[0px_1px_1.5px_rgba(18,9,0,0.1)] hover:bg-[var(--Surface-UI-surface-Surface-Universal-alternate)] transition-colors"
          >
            <span>Add Skill</span>
