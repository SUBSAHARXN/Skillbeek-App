...
export function SkillSelectView({
  ...
  return (
    <div className="w-full max-w-[384px] h-[812px] bg-[var(--Surface-Primary-Background)] rounded-[32px] overflow-hidden relative flex flex-col mx-auto shadow-2xl">
      ...
      {!isViewOnly && (
        <div className="w-full px-[16px] flex justify-between items-center pt-[16px] mb-[40px] shrink-0 relative z-10 bg-[var(--Surface-Primary-Background)]">
          <button
            onClick={() => setIsSaveModalOpen(true)}
            className="h-[44px] px-[16px] border-2 border-[var(--Button-Primary-Stroke-Stroke-default)] hover:border-[var(--Text-Primary-Subtitle)] active:border-[var(--Button-Primary-Stroke-Stroke-tertiary-default)] rounded-[99px] flex items-center justify-center transition-colors"
          >
            <span className="font-['Nunito'] font-bold text-[var(--Text-Primary-Body)] text-[16px]">
              Save and Exit
            </span>
          </button>
          <button className="h-[44px] px-[16px] border-2 border-[var(--Button-Primary-Stroke-Stroke-default)] hover:border-[var(--Text-Primary-Subtitle)] active:border-[var(--Button-Primary-Stroke-Stroke-tertiary-default)] rounded-[99px] flex items-center justify-center transition-colors">
            <span className="font-['Nunito'] font-bold text-[var(--Text-Primary-Body)] text-[16px]">
              Questions?
            </span>
          </button>
      ...
      {isViewOnly && (
        <div className="w-full px-[16px] flex items-center pt-[16px] mb-[40px] shrink-0 relative z-10 bg-[var(--Surface-Primary-Background)]">
          <button
            onClick={onBack}
      ...
      {/* Search Bar Area */}
      <div className="w-full px-[16px] pb-[32px] shrink-0 z-10 relative bg-[var(--Surface-Primary-Background)]">
        ...
        >
          <div className="flex gap-[13px] overflow-x-auto scrollbar-hide pb-[2px]">
            {appliedFilters.map(filter => (
              <div
                key={filter}
                className="flex items-center gap-[12px] bg-[var(--Surface-UI-surface-surface-elevated)] rounded-[12px] px-[12px] py-[12px] shrink-0"
              >
      ...
      >
        {filteredCategories.map((category) => (
          <div key={category.name} className="flex flex-col gap-[6px]">
            <div className="py-[8px]">
              <h2 className="font-['Nunito'] font-extrabold text-[var(--Text-Primary-heading-1)] text-[14px] leading-[20px] tracking-[1px] uppercase">
                {category.name}
              </h2>
            </div>
              ...
              {category.skills.map((skill) => {
                ...
                return (
                  <div
                    ...
                    className={`w-full bg-[var(--Surface-UI-surface-surface-elevated)] rounded-[12px] shadow-[0px_1px_1.5px_rgba(18,9,0,0.1)] p-[8px] flex items-center justify-between select-none transition-all ${
                      isViewOnly ? "h-[60px]" : ""
                    } ${isDisabled
                      ? "opacity-40 cursor-not-allowed"
                      : "cursor-pointer active:scale-[0.99] hover:bg-[var(--Surface-UI-surface-surface-elevated)]"
                      }`}
                  >
        ...
        {filteredCategories.length === 0 && (
          <div className="flex justify-center py-[40px] text-[var(--Text-Primary-Text-placeholder)] font-['Nunito'] font-medium">
      ...
      {/* Overlay Background */}
      <div
        className={`absolute inset-0 z-30 bg-[var(--Surface-UI-surface-Background)]/[0.15] backdrop-blur-[2px] transition-opacity duration-300 ${isFilterModalOpen ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"
          }`}
      ...
      <div
        className={`absolute bottom-0 left-0 w-full bg-[var(--Surface-UI-surface-surface-elevated)] rounded-t-[24px] flex flex-col pt-[8px] pb-[44px] z-40 transition-transform duration-300 ${isFilterModalOpen ? "translate-y-0 ease-[cubic-bezier(0,0,0.2,1)]" : "translate-y-full ease-[cubic-bezier(0.4,0,1,1)]"
          }`}
      >
        <div className="w-full flex flex-col items-center gap-[16px] px-[16px]">
          {/* Drag Handle */}
          <div className="w-[64px] h-[8px] bg-[var(--Surface-UI-surface-surface-elevated)] rounded-[4px] mt-[8px]" />
